...
size=(1080,720) # The screen size
screen = display.set_mode(size) # The screen as a suface

## Fonts

comicFont = font.SysFont("Times New Roman",26) ## this is the font used for the texts
...
####################################################
## MUSIC
music = ["music/Dragonballsuper.wav","music/BeerusMadness.wav", "music/Ultra Instinct Reborn.wav",
...
while running:
    ...
    for evt in event.get():
        ...
        if evt.type == MOUSEBUTTONDOWN:
            ...
            if evt.button == 1 and nextRect.collidepoint(mx,my) and b <= len(music):
                b = b+1
            ...
            if evt.button == 1 and playRect.collidepoint(mx,my) and music2 == "false":
                mixer.music.stop()
                music2 = "true"

            elif evt.button == 1 and playRect.collidepoint(mx,my) and music2 == "true":
                mixer.music.play(-1)
                music2 = "false"
...
###################################################################
    # posText = comicFont.render("Pos:",True,(0,0,0))
    # xposText = comicFont.render("X=",True,(0,0,0))
    # yposText = comicFont.render("Y=",True,(0,0,0))
    ...
    # screen.blit(posPic,(206,572))
    # screen.blit(XposPic, (256,572))
    # screen.blit(xPic, (276,572))
                    ...
                    if pixel!=col:
                        while len(pixelpos)>0:#continously loops until all pixels are the same colour
                            if canvasRect.collidepoint(pixelpos[0]) and screen.get_at(pixelpos[0])== pixel:
                                ...
                                pixelpos.append((pixelpos[0][0],pixelpos[0][1]+1))  #Adds four pixels to the list
